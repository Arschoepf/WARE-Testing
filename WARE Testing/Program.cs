...
namespace WavHeaderScanner
{
    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            {
                Console.WriteLine($"File not found: {filePath} Press any key to exit.");
                Console.ReadKey();
                return;
            }
            ...
            try
            {
                ...
                using (BinaryReader reader = new BinaryReader(fs))
                {
                    ...
                    if (scotEntry != null && !dataRead)
                    {
                        ...
                        startHour = (byte)(reader.ReadByte() - 0x80);                                       // 1 byte, start hour (0-23 stored as 0x80-0x97)
                        endHour = (byte)(reader.ReadByte() - 0x80);                                         // 1 byte, end hour (0-23 stored as 0x80-0x97)
                        ...
                        fileInfo.ReleaseDate = new DateTime(1,1,int.Parse(new string(reader.ReadChars(4)).TrimEnd('\0')));                      // 4 bytes
                    ...
                    if (listEntry != null && !dataRead)
                    {
                        ...
                        if (new string(reader.ReadChars(4)).Equals("INFO"))
                        {
                            ...
                            while (fs.Position < listEntry.Offset + listEntry.Size)
                            {
                                ...
                                Console.WriteLine($"{infoId}: {infoData}");
                                // Handle padding
                    ...
                    if (!DateTime.TryParseExact(rawStartDate, "yyMMdd", null, System.Globalization.DateTimeStyles.None, out tempStartDate))
                    {
                        fileInfo.StartDate = DateTime.UnixEpoch;
                    }
                    else
                    {
                        fileInfo.StartDate.AddHours(startHour);
                    }

                    DateTime endDate;
                    if (!DateTime.TryParseExact(rawEndDate, "yyMMdd", null, System.Globalization.DateTimeStyles.None, out tempEndDate))
                    {
                        fileInfo.EndDate = DateTime.MaxValue;
                    }
                    else
                    {
                        fileInfo.EndDate.AddHours(endHour);
                    }
                    ...
                    using (var writer = new BinaryWriter(File.Create(Path.Combine(targetFolder, "testout.wav"))))
                    {
